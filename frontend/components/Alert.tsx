...
const Alert = ({ type, header, message, onClose }: AlertProps): JSX.Element => {
  return (
    <>
      {type == AlertTypes.Error && (
        <div
          id="alert-2"
          className="flex p-4 bg-red-100 px-6 py-4 mb-4 relative w-full rounded-lg dark:bg-red-200"
          role="alert"
        >
          <svg
          ...
          <div className="ml-3 text-sm font-medium text-red-700 dark:text-red-800">
            <b className="capitalize">{header}</b> {message}
          </div>
          <button
            type="button"
            onClick={onClose}
      ...
      {type == AlertTypes.Success && (
        <div
          id="alert-3"
          className="flex p-4 bg-green-100 px-6 py-4 mb-4 relative w-full rounded-lg dark:bg-green-200"
          role="alert"
        >
          <svg
